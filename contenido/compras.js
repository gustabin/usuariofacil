    ...
    $.ajax({
        ...
        success: function (productos) {
            ...
            $.each(productos, function (_, producto) {
                const cardId = `productoCard_${producto.ProductoID}`;
                const card = $(`<div id="${cardId}" class="col">`).html(`
                <div class="card mb-4 rounded-3 shadow-sm">
                    ...
                    <ul class="list-unstyled mt-3 mb-4">
                            <li id="productoStock_${producto.ProductoID}">${producto.Stock} unidades en stock</li>
                        </ul>
...
$(document).on('click', '.agregarAlCarritoBtn', function () {
    ...
    mostrarCarrito();
    actualizarStockEnFront(productoId);
})
...
function actualizarTarjetaProducto(producto) {
    ...
    const stockElementId = `#productoStock_${producto.ProductoID}`;
    $(stockElementId).text(`${producto.Stock} unidades en stock`);

    // Actualizar la información en la tarjeta
    tarjetaProducto.find('.card-img-top').attr('src', producto.ImagenURL);
